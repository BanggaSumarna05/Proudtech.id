<x-frontend-layout>
    ...
    <section class="pt-8 md:pt-14 pb-16 md:pb-40 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-5 sm:px-6 lg:px-10 text-center relative z-10">
            ...
            <p data-aos="fade-up" data-aos-delay="200"
                class="text-base sm:text-xl md:text-2xl text-slate-400 max-w-3xl mx-auto font-medium leading-relaxed px-2">
    ...
    <!-- Core Narrative -->
    <section class="py-12 sm:py-16 md:py-24 bg-[#080808] border-y border-white/[0.03]">
        <div class="max-w-7xl mx-auto px-5 sm:px-6 lg:px-10">
            <div class="flex flex-col lg:flex-row items-center gap-12 sm:gap-16 md:gap-24">
                <div data-aos="fade-right" class="lg:w-1/2 relative w-full">
                    <div class="absolute -inset-4 bg-blue-600/10 rounded-[3rem] blur-2xl"></div>
                    <div class="relative rounded-[2rem] sm:rounded-[3rem] overflow-hidden border border-white/10 group">
                ...
                <div data-aos="fade-left" class="lg:w-1/2 space-y-8 sm:space-y-10 md:space-y-12">
                    ...
                    <h2
                        class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white tracking-tighter uppercase leading-[0.9]">
                        {!! \App\Models\Setting::get(
                    ...
                    <p class="text-gray-400 text-base sm:text-lg md:text-xl font-medium leading-relaxed">
                        {!! \App\Models\Setting::get(
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 md:gap-10">
                        <div class="space-y-3 sm:space-y-4 group">
                            <h4
    ...
    <!-- Standards -->
    <section class="py-12 sm:py-16 md:py-40 relative">
        <div class="max-w-7xl mx-auto px-5 sm:px-6 lg:px-10">
            <div class="text-center mb-12 sm:mb-16 md:mb-24">
                <h2 class="text-blue-500 font-black uppercase tracking-[0.4em] text-xs mb-4 sm:mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
                @foreach ($standards as $index => $standard)
                    <div data-aos="fade-up" data-aos-delay="{{ $index * 100 }}"
                        class="p-7 sm:p-8 md:p-12 rounded-[2rem] sm:rounded-[2.5rem] glass border border-white/[0.05] hover:border-{{ $standard->color }}-500/30 transition-all duration-700 group">
                        <div
                            class="w-13 h-13 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-{{ $standard->color }}-600/10 border border-{{ $standard->color }}-500/20 flex items-center justify-center text-{{ $standard->color }}-500 mb-7 sm:mb-10 group-hover:scale-110 transition-transform">
                            <i class="{{ $standard->icon }} text-xl sm:text-2xl"></i>
                        </div>
                        <h3 class="text-xl sm:text-2xl font-black text-white mb-3 sm:mb-4 uppercase tracking-tight">
                            {{ $standard->title }}</h3>
                        <p

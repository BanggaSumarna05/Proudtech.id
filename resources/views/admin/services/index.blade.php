<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h3 class="text-lg font-bold text-white">All Services</h3>
                <p class="text-sm text-gray-500">Manage the digital solutions offered by Proud Tech.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-white/[0.02] border-b border-white/5">
                            ...
                            <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Service
                            </th>
                            <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Status
                            </th>
                            <th
                            ...
                            <tr class="hover:bg-white/[0.01] transition-colors">
                                <td class="px-6 py-4">
                                    <span
                                        ...
                                        {{ str_pad($service->order, 2, '0', STR_PAD_LEFT) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-white">{{ $service->title }}</p>
                                            <p class="text-xs text-gray-500">{{ Str::limit($service->description, 40) }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-500/10 text-red-400 text-[10px] font-bold border border-red-500/20">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <form action="{{ route('admin.services.destroy', $service) }}" method="POST"
                                            class="inline-block" onsubmit="return confirm('Archive this service?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"

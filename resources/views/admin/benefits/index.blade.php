<x-app-layout>
    <x-slot name="header">Benefits Management</x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h3 class="text-lg font-bold text-white">Value Proposition</h3>
                <p class="text-sm text-gray-500">Manage the "Why Us" section on your homepage.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-white/[0.02] border-b border-white/5">
                            ...
                            <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Benefit
                            </th>
                            <th
                            ...
                            <tr class="hover:bg-white/[0.01] transition-colors">
                                <td class="px-6 py-4">
                                    <span
                                        ...
                                        {{ str_pad($benefit->order, 2, '0', STR_PAD_LEFT) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-white">{{ $benefit->title }}</p>
                                            <p class="text-xs text-gray-500">{{ Str::limit($benefit->description, 60) }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <form action="{{ route('admin.benefits.destroy', $benefit) }}" method="POST"
                                            class="inline-block" onsubmit="return confirm('Delete this benefit?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <p class="text-gray-500 font-medium">No benefits listed yet.</p>
                                        <a href="{{ route('admin.benefits.create') }}"

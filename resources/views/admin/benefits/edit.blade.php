<x-app-layout>
    <x-slot name="header">Edit Benefit</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-[#161923] rounded-3xl border border-white/5 p-8 shadow-xl">
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h4 class="text-white font-bold">Edit Value Blueprint</h4>
                    <p class="text-xs text-gray-500">Modifying the "{{ $benefit->title }}" value proposition.</p>
                </div>
            </div>

            <form method="POST" action="{{ route('admin.benefits.update', $benefit) }}" class="space-y-8">
                @csrf
                @method('PATCH')
